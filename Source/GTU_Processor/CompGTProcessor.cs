...
namespace GTU_Processor
{
    public class CompGTProcessor : ThingComp
    {
        ...
        {
            string str = null;
            if (!Full && Props.writeRequiredIngredients)
            {
                str = ingredientStored + "/" + Props.ingredientCount + " " + Props.thingIngredient.label;
            }
            else if (Full && !Completed && Props.writeTimeLeftToProcess)
            {
                var progInt = 0f;
            ...
            else if (Completed && !Props.spawnOnFloor && Props.writeTimeLeftToProcess)
            {
